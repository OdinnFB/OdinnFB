...
# Use BCM pin 14
LED_PIN = 14
PWM_FREQ = 800  # Hz (balanced for both hardware and software PWM)
PWM_MIN_DELTA = 0.5  # skip duty updates that change less than this percent
INVERT_BRIGHTNESS = True  # hardware wiring uses active-low LED
...
def set_pwm_duty_percent(percent: float) -> None:
    ...
    """
    global _current_pwm_percent
    percent = max(0.0, min(100.0, float(percent)))
    if abs(percent - _current_pwm_percent) < PWM_MIN_DELTA:
        return
    _current_pwm_percent = percent
